...
gj.widget = function () {
	...
	height: function (a, b) {
		...
		return "border-box" === d.boxSizing ? c = parseInt(d.height, 10) : (c = parseInt(d.height, 10), c += parseInt(d.paddingTop || 0, 10) + parseInt(d.paddingBottom || 0, 10), c += parseInt(d.borderTop || 0, 10) + parseInt(d.borderBottom || 0, 10)),
	...
	width: function (a, b) {
		var c,
		d = window.getComputedStyle(a);
		return "border-box" === d.boxSizing ? c = parseInt(d.width, 10) : (c = parseInt(d.width, 10), c += parseInt(d.paddingLeft || 0, 10) + parseInt(d.paddingRight || 0, 10), c += parseInt(d.borderLeft || 0, 10) + parseInt(d.borderRight || 0, 10)),
	...
	position: function (a, b, c) {
		...
		return b && (l += parseInt(f.paddingTop || 0, 10), m += parseInt(f.paddingLeft || 0, 10)),
		c && (l += parseInt(f.marginTop || 0, 10), m += parseInt(f.marginLeft || 0, 10)), {
			top: l,
			left: m,
	...
	renderHeader: function (a) {
		...
		0 === d.length && e.closeButtonInHeader ? (d = $('<button type="button" data-role="close" title="' + gj.dialog.messages[e.locale].Close + '"><span>×</span></button>'), d.addClass(e.style.headerCloseButton), b.append(d)) : d.length > 0 && !1 === e.closeButtonInHeader ? d.hide() : d.addClass(e.style.headerCloseButton),
		b
	},
...
(jQuery), gj.dialog.messages["en-us"] = {
	...
	move: function (a, b, c, d, e, f) {
		...
		!1 !== gj.draggable.events.drag($(a), k, l, e, f) && (a.style.top = k + "px", a.style.left = l + "px")
	},
	destroy: function (a) {
		return "true" === a.attr("data-draggable") && (gj.documentManager.unsubscribeForEvent("mouseup", a.data("guid")), a.removeData(), a.removeAttr("data-guid"), a.removeAttr("data-draggable"), a.off("drag").off("start").off("stop"), gj.draggable.methods.getHandleElement(a).off("mousedown")),
		a
	}
}, gj.draggable.events = {
...
(jQuery), gj.grid = {
	...
	formatText: function (a, b) {
		return a = a && "date" === b.type ? gj.core.formatDate(gj.core.parseDate(a, b.format), b.format) : void 0 === a || null === a ? "" : a.toString(),
	...
	setRecordsData: function (a, b) {
		...
		d = 0,
		e = a.data();
		return $.isArray(b) ? (c = b, d = b.length) : e && e.mapping && $.isArray(b[e.mapping.dataField]) && (c = b[e.mapping.dataField], (d = b[e.mapping.totalRecordsField]) && !isNaN(d) || (d = 0)),
		a.data("records", c),
...
(jQuery), gj.grid.plugins.fixedHeader = {
		...
		getCSV: function (a) {
			...
			if (g.length) {
				for (b = 0; b < f.length; b++)
					!0 !== f[b].hidden && (d += '"' + (f[b].title || f[b].field) + '",');
	...
	render: function (a, b) {
		return b && ("string" == typeof b && JSON && (b = JSON.parse(b)), a.data("records", gj.tree.methods.getRecords(a, b)), gj.tree.methods.loadData(a)),
		a
	},
	filter: function (a) {
	...
	getRecords: function (a, b) {
		...
		for (c = 0; c < b.length; c++)
			d = g.primaryKey && b[c][g.primaryKey] ? b[c][g.primaryKey] : g.autoGenId++, e = {
				id: d,
				data: b[c]
			},
		b[c][g.childrenField] && b[c][g.childrenField].length ? (e.children = gj.tree.methods.getRecords(a, b[c][g.childrenField]), delete b[c][g.childrenField]) : e.children = [],
		f.push(e);
		return f
	},
	loadData: function (a) {
	...
	appendNode: function (a, b, c, d, e) {
		...
		if (k.indentation && l.append('<span data-role="spacer" style="width: ' + k.indentation * (d - 1) + 'px;"></span>'), p ? gj.tree.methods.disableNode(a, g) : (m.on("click", gj.tree.methods.expanderClickHandler(a)), n.on("click", gj.tree.methods.displayClickHandler(a))), l.append(m), l.append(n), g.append(l), e ? b.find("li:eq(" + (e - 1) + ")").before(g) : b.append(g), c.children.length || o)
			for (m.empty().append(k.icons.expand), h = $("<ul />").addClass(k.style.list).addClass("gj-hidden"), g.append(h), f = 0; f < c.children.length; f++)
				gj.tree.methods.appendNode(a, h, c.children[f], d + 1);
		else
			k.style.leafIcon ? m.addClass(k.style.leafIcon) : m.html("&nbsp;");
		k.imageCssClassField && c.data[k.imageCssClassField] ? $('<span data-role="image"><span class="' + c.data[k.imageCssClassField] + '"></span></span>').insertBefore(n) : k.imageUrlField && c.data[k.imageUrlField] ? (i = $('<span data-role="image"></span>'), i.insertBefore(n), j = $('<img src="' + c.data[k.imageUrlField] + '"></img>'), j.attr("width", i.width()).attr("height", i.height()), i.append(j)) : k.imageHtmlField && c.data[k.imageHtmlField] && (i = $('<span data-role="image">' + c.data[k.imageHtmlField] + "</span>"), i.insertBefore(n)),
	...
	getById: function (a, b, c) {
		...
		for (d = 0; d < c.length; d++) {
			...
			if (c[d].children && c[d].children.length && (e = gj.tree.methods.getById(a, b, c[d].children)))
				break
		}
		return e
	},
	...
	getDataByText: function (a, b, c) {
		...
		for (d = 0; d < c.length; d++) {
			...
			if (c[d].children && c[d].children.length && (e = gj.tree.methods.getDataByText(a, b, c[d].children)))
				break
		}
		return e
	},
	...
	getAll: function (a, b) {
		...
		for (c = 0; c < b.length; c++)
			d = JSON.parse(JSON.stringify(b[c].data)), b[c].children.length && (d[f] = gj.tree.methods.getAll(a, b[c].children)), e.push(d);
		return e
	},
	addNode: function (a, b, c, d) {
		var e,
		f = gj.tree.methods.getRecords(a, [b])[0];
		return c && c.length ? ("li" === c[0].tagName.toLowerCase() && (0 === c.children("ul").length && (c.find('[data-role="expander"]').empty().append(a.data().icons.collapse), c.append($("<ul />").addClass(a.data().style.list))), c = c.children("ul")), gj.tree.methods.getById(a, c.parent().data("id"), a.data("records")).children.push(f)) : (c = a.children("ul"), a.data("records").push(f)),
			...
			return function (c, g) {
				...
				d.each(function () {
					...
					if (i.droppable("isOver", g))
						return c = i.closest("li"), d = b.parent("ul").parent("li"), e = c.children("ul"), 0 === e.length && (e = $("<ul />").addClass(f.style.list), c.append(e)), !1 !== gj.tree.plugins.dragAndDrop.events.nodeDrop(a, b.data("id"), c.data("id"), e.children("li").length + 1) && (e.append(b), gj.tree.plugins.dragAndDrop.private.refresh(a, b, c, d)), h = !0, !1;
					i.droppable("destroy")
				}),
				h || e.each(function () {
					var c,
		...
		createDoneHandler: function (a, b) {
			return function (c) {
				...
				if ("string" == typeof c && JSON && (c = JSON.parse(c)), c && c.length) {
					for (f = $("<ul />").addClass(g.style.list), b.append(f), d = 0; d < c.length; d++)
						a.addNode(c[d], f);
...
(jQuery), gj.editor.messages["en-us"] = {
	...
	setListPosition: function (a, b, c) {
		...
		"auto" === c.maxHeight ? h.top < g.top ? h.top + d + e > window.innerHeight && (f = window.innerHeight - h.top - e - 3) : h.top - d - 3 > 0 ? b.style.top = Math.round(h.top + i - d - 3) + "px" : (b.style.top = i + "px", f = h.top - 3) : !isNaN(c.maxHeight) && c.maxHeight < d && (f = c.maxHeight),
	...
	close: function (a, b) {
		var c = a.data();
		a.parent().find('[role="expander"]').html(c.icons.dropdown),
		b.hide()
	},
	select: function (a, b) {
		var c = a.data(),
		...
		return f && (d.children("li").removeClass(c.style.active), e.addClass(c.style.active), a[0].value = b, a.next('[role="presenter"]').find('[role="display"]').html(f[c.textField]), gj.dropdown.events.change(a)),
...
(jQuery), gj.timepicker = {
	...
	init: function (a) {
		return gj.widget.prototype.init.call(this, a, "timepicker"),
	...
	initialize: function (a, b) {
		...
		c = "bootstrap" === b.uiLibrary ? $('<span class="input-group-addon">' + b.icons.rightIcon + "</span>") : "bootstrap4" === b.uiLibrary ? $('<span class="input-group-append"><button class="btn btn-outline-secondary border-left-0 border" type="button">' + b.icons.rightIcon + "</button></span>") : $(b.icons.rightIcon),
		c.attr("role", "right-icon"),
		0 === d.length ? (d = $('<div role="wrapper" />').addClass(b.style.wrapper), a.wrap(d)) : d.addClass(b.style.wrapper),
		d = a.parent('div[role="wrapper"]'),
		b.width && d.css("width", b.width),
		a.val(b.value).addClass(b.style.input).attr("role", "input"),
		"bootstrap" === b.uiLibrary || "bootstrap4" === b.uiLibrary ? "small" === b.size ? (d.addClass("input-group-sm"), a.addClass("form-control-sm")) : "large" === b.size && (d.addClass("input-group-lg"), a.addClass("form-control-lg")) : "small" === b.size ? d.addClass("small") : "large" === b.size && d.addClass("large"),
	...
	updateArrow: function (a, b, c, d) {
		...
		"hours" == d.dialMode ? (f = gj.timepicker.methods.getPointerValue(g - j - e.left, h - i - e.top, d.mode), c.attr("hour", "ampm" === d.mode && "pm" === c.attr("mode") && f < 12 ? f + 12 : f)) : "minutes" == d.dialMode && (f = gj.timepicker.methods.getPointerValue(g - j - e.left, h - i - e.top, "minutes"), c.attr("minute", f)),
	...
	destroy: function (a) {
		...
		return b && (a.off(), d.parent('[role="modal"]').length > 0 && d.unwrap(), d.remove(), a.removeData(), a.removeAttr("data-type").removeAttr("data-guid").removeAttr("data-timepicker"), a.removeClass(), c.children('[role="right-icon"]').remove(), a.unwrap()),
		a
	}
	...
	a
}, gj.timepicker.widget.prototype = new gj.widget, gj.timepicker.widget.constructor = gj.timepicker.widget, function (a) {
